...
def cg(f_Ax, b, x0=0, cg_iters=10, callback=None, verbose=False, residual_tol=1e-10):
    """
    Demmel p 312
    """
...
class ConjugateGradientOptimizer(Serializable):
    ...
    def __init__(
        ...
        self.last_congrad = 0 #this saves the last conjugated gradient for the next round. 
    ...
    def optimize(self, inputs, extra_inputs=None, subsample_grouped_inputs=None):
        ...
        logger.record_tabular('xHx', descent_direction.dot(Hx(descent_direction)))
        logger.record_tabular('xTx', flat_g.dot(flat_g))
        logger.record_tabular('min flatgrad', min(abs(flat_g)))
